...
def view(recipe_id, slug):
    ...
    Displays the recipe details if is found, if not a redirect to the index
    page is triggered.

    Args:
        recipe_id: The id of recipe to display
        slug: Recipe title but only with valid url chars. Used for SEO reasons
                only.
    """
...
def new():
    ...
    if request.method == 'GET':
        ...
        recipe = Recipe(
            current_user, form.category.data, form.title.data,
            form.description.data, form.ingredients.data, form.directions.data,
            form.prep_time.data, form.cook_time.data, form.servings.data
        )
        db.session.add(recipe)

        for imagedata in [
            form.image1.data, form.image2.data, form.image3.data
        ]:
        ...
        return redirect(
            url_for('recipes.view', recipe_id=recipe.id, slug=recipe.slug)
        )
    else:
...
def edit(recipe_id):
    ...
    if request.method == 'GET':
        ...
        flash('Oopss! You are not allowed to edit this recipe!', 'danger')
    elif form.validate_on_submit():
    ...
    return render_template(
        'recipes/edit.html',
        form=form, recipe_id=recipe.id, images=recipe.images
    )
...
def delete(recipe_id):
    ...
    Deletes the recipe from the database if exists. All the associated images
    will also be deleted.
    This page can be accessed only by logged in users and CSRF check runs for
    each delete.
    ...
    if recipe.user.id == current_user.id:
        ...
        flash(
            'Recipe "%s" was successfully deleted!' % recipe.title, 'success'
        )
        db.session.commit()
        return 'success'

    flash('Oopss! You are not allwed to delete this recipe!', 'danger')
    return 'error'


@mod.route(
    '/<int:recipe_id>/image/<int:image_id>/delete', methods=['GET', 'POST']
)
@login_required
...
def category(category_id, slug):
    ...
    return render_template(
        'recipes/by_category.html', recipes=recipes, category=category
    )
...
def json():
    ...
    recipes = db.session.query(Recipe) \
        .order_by(Recipe.category_id.asc(), Recipe.title.asc())
...
def xml():
    ...
    recipes = db.session.query(Recipe) \
        .order_by(Recipe.category_id.asc(), Recipe.title.asc())
    xml = unparse(
        dict(recipes=dict(recipe=[recipe.to_dict() for recipe in recipes]))
    )
